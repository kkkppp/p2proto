...
export default function TableForm({
    ...
    const validatePasswords = () => {
        ...
        fields.forEach((f) => {
            ...
            const confVal = confirm[f.name];

            const isEditMode = mode === 'edit';

            // Did the user actually edit the password?
            const userChangedPassword =
                !isEditMode || // in create mode it's always "changed"
                (newVal !== PASSWORD_MASK || confVal !== undefined);

            if (!userChangedPassword) {
                // password left untouched in edit mode -> fine, skip
                return;
            }

            // Now we ARE changing password. Validate:
            if (!newVal) {
                invalid[f.name] = ['Password cannot be empty.'];
                return;
            }
            if (newVal !== confVal) {
    ...
    const buildPayload = () => {
        // Build list of fields to actually send
        const outgoingFields = [];

        fields.forEach(({ name, type, required, autoGenerated, value }) => {
            if (type === 'PASSWORD' && mode === 'edit') {
                const confVal = confirm[name];

                const userChangedPassword =
                    (value ?? '') !== PASSWORD_MASK || confVal !== undefined;

                if (!userChangedPassword) {
                    // user did NOT change password -> skip pushing this field entirely
                    return;
                }

                // user DID change password -> include it with the new value
                outgoingFields.push({
                    name,
                    type,
                    required,
            ...
            // non-password fields OR create mode passwords:
            outgoingFields.push({
                name,
                type,
                required,
